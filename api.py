# ==============================================================================
# FELIX AI SOLUTIONS - API DEL CANDIDATO DIGITAL (v29.0 "MAESTRO")
# Versión de producción final con un prompt refinado para un equilibrio
# óptimo entre honestidad, naturalidad y profundidad conversacional.
# ==============================================================================
...
def whatsapp_reply():
    try:
        ...
        # Guardia 4: Moderación de Contenido (Silencio Profesional)
        try:
            mod_response = openai.Moderation.create(input=incoming_msg)
            if mod_response['results'][0]['flagged']:
                print(f"Mensaje flageado por moderación y descartado silenciosamente: {incoming_msg}")
                # NO enviamos respuesta para no entrar en un bucle con un usuario tóxico.
                # Pero SÍ lo registramos para que el equipo de campaña esté al tanto.
                log_data.update({'retrieved_context': 'MODERADO', 'ai_response': 'SILENCIO_PROFESIONAL', 'moderation_flags': 'FLAGEADO', 'response_time_ms': int((time.time() - start_time) * 1000), 'topic': 'Tóxico', 'sentiment': 'Negativo'})
                update_cumulative_log_on_hf(log_data)
                return str(MessagingResponse()) # Devolvemos una respuesta vacía para terminar la interacción.
        except Exception as e:
