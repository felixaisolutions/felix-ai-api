# ==============================================================================
# FELIX AI SOLUTIONS - API DEL CANDIDATO DIGITAL (v25.0 "ARQUITECTURA FINAL")
# Versión final que integra todas las mejoras y corrige el orden lógico de las
# guardias de seguridad para darles la máxima prioridad.
# ==============================================================================
...
IDENTITY_KEYWORDS = ["robot", "ia", "i.a.", "inteligencia artificial", "máquina", "maquina", "bot", "programa", "real", "persona", "de verdad"]
...
*Aviso Importante: Soy un agente de inteligencia artificial. Aunque me esfuerzo por ser preciso, la información proporcionada es de carácter informativo y puede contener imprecisiones. Mis respuestas no constituyen un compromiso vinculante por parte del candidato.*
"""
...
He sido entrenado con su plan de gobierno y su visión para el Quindío para poder responder tus preguntas de la manera más fiel posible a su pensamiento. Sin embargo, soy un programa informático y puedo cometer errores.
...
FALLBACK_RESPONSES = [
    ...
    "Mi conocimiento se centra en mi plan de gobierno. No tengo una respuesta para esa consulta en particular. ¿Hay algo más en lo que pueda ayudarte sobre mis propuestas?",
    "No tengo claro a qué te refieres. ¿Podrías darme un poco más de contexto o hacer la pregunta de un modo diferente?"
]
...
def ask_candidato_ia(pregunta):
    ...
    if contexto == "SIN_CONTEXTO":
        respuesta_final = random.choice(FALLBACK_RESPONSES)
        return respuesta_final, "SIN_CONTEXTO"
    ...
    Tu Misión: Responder la pregunta del ciudadano basándote ÚNICA y ESTRICTAMENTE en la "Información de Apoyo".
    Instrucciones Clave:
    1.  **REGLA DE ORO DE VERIFICACIÓN:** Antes de responder, verifica si la pregunta del usuario (ej: '¿es cierto que usted fue X?') se puede confirmar DIRECTAMENTE con la 'Información de Apoyo'. Si el contexto NO contiene esa información biográfica o ese hecho específico, DEBES iniciar tu respuesta diciendo que esa información no es correcta o no se encuentra en tu plan. **NO CONFIRMES NUNCA UN HECHO QUE NO ESTÉ EXPLÍCITAMENTE ESCRITO EN EL TEXTO DE APOYO.** Solo después de aclarar esto, puedes ofrecer información relacionada que SÍ esté en el contexto.
    2.  **Sintetiza y Conecta:** Lee TODA la "Información de Apoyo" y conecta las diferentes ideas para dar una respuesta completa y natural.
    3.  **AUMENTA CON HECHOS:** Enriquece tu respuesta mencionando nombres de programas, cifras o datos concretos que encuentres en la "Información de Apoyo".
    4.  **Brevedad Esencial:** Sé conciso. Responde en un máximo de 2 o 3 párrafos cortos.
    ---
    Información de Apoyo: {contexto}
    ---
    Pregunta del Ciudadano: "{pregunta}"
    Respuesta de Javier Montoya:
    """
    try:
        res_completion = openai.ChatCompletion.create(model="gpt-4o-mini", messages=[{"role": "user", "content": prompt_template}], temperature=0.3)
        respuesta_final = res_completion['choices'][0]['message']['content']
...
def whatsapp_reply():
    try:
        ...
        print(f"Mensaje recibido de {user_number}: '{incoming_msg}' (Media: {media_items})")

        # Guardia 1: Multimedia
        if int(media_items) > 0:
            resp = MessagingResponse(); resp.message(MEDIA_FALLBACK_MESSAGE)
            log_data = {'timestamp': datetime.utcnow().isoformat(), 'user_id': user_number, 'user_query': "[Archivo Multimedia]", 'retrieved_context': "MULTIMEDIA", 'ai_response': MEDIA_FALLBACK_MESSAGE, 'moderation_flags': "N/A", 'response_time_ms': int((time.time() - start_time) * 1000), 'topic': 'Otro', 'sentiment': 'Neutro'}
            update_cumulative_log_on_hf(log_data)
            return str(resp)

        # Guardia 2: Identidad
        identity_matches = sum(1 for keyword in IDENTITY_KEYWORDS if keyword in incoming_msg.lower())
        if "quién eres" in incoming_msg.lower() or "quien eres" in incoming_msg.lower() or identity_matches >= 2:
            resp = MessagingResponse(); resp.message(IDENTITY_FALLBACK_MESSAGE)
            log_data = {'timestamp': datetime.utcnow().isoformat(), 'user_id': user_number, 'user_query': incoming_msg, 'retrieved_context': "IDENTIDAD", 'ai_response': IDENTITY_FALLBACK_MESSAGE, 'moderation_flags': "N/A", 'response_time_ms': int((time.time() - start_time) * 1000), 'topic': 'Sobre el Candidato', 'sentiment': 'Neutro'}
            update_cumulative_log_on_hf(log_data)
            return str(resp)
        ...
        if is_new_session and is_greeting:
            ...
            resp = MessagingResponse(); resp.message(WELCOME_MESSAGE)
            log_data = {'timestamp': datetime.utcnow().isoformat(), 'user_id': user_number, 'user_query': incoming_msg, 'retrieved_context': "BIENVENIDA", 'ai_response': WELCOME_MESSAGE, 'moderation_flags': "N/A", 'response_time_ms': int((time.time() - start_time) * 1000), 'topic': 'Saludo/Otro', 'sentiment': 'Neutro'}
            update_cumulative_log_on_hf(log_data)
            return str(resp)

        # Guardia 4: Moderación de Contenido
        try:
            mod_response = openai.Moderation.create(input=incoming_msg)
            if mod_response['results'][0]['flagged']:
                print(f"Mensaje flageado por moderación: {incoming_msg}")
                safe_response = "Entiendo que este es un tema de gran interés, pero mi enfoque principal está en las propuestas de mi plan de gobierno."
                resp = MessagingResponse(); resp.message(safe_response)
                log_data = {'timestamp': datetime.utcnow().isoformat(), 'user_id': user_number, 'user_query': incoming_msg, 'retrieved_context': "MODERADO", 'ai_response': safe_response, 'moderation_flags': "FLAGEADO", 'response_time_ms': int((time.time() - start_time) * 1000), 'topic': 'Sensible', 'sentiment': 'Negativo'}
                update_cumulative_log_on_hf(log_data)
                return str(resp)
        except Exception as e:
        ...
        log_data = {
            'timestamp': datetime.utcnow().isoformat(), 'user_id': user_number,
            'user_query': incoming_msg, 'retrieved_context': contexto_recuperado,
            'ai_response': respuesta_ia, 'moderation_flags': "OK",
