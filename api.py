# ==============================================================================
# FELIX AI SOLUTIONS - API DEL CANDIDATO DIGITAL (VERSIÓN 17.2 - FUSIÓN FINAL Y ROBUSTA)
# Combina la lógica funcional v17.0 con el manejo de errores mejorado.
# ==============================================================================
...
# --- Importaciones para la subida a Hugging Face ---
...
if HF_TOKEN:
    ...
    print("ADVERTENCIA: No se encontró el HUGGING_FACE_TOKEN. La subida de logs está desactivada.")

client_chroma = chromadb.PersistentClient(path="./chroma_db")
collection = client_chroma.get_or_create_collection(name="candidato_ia_stable")
CEREBRO_CARGADO = False
LOG_FILE = 'log.csv'
LOG_HEADERS = ['timestamp', 'user_id', 'user_query', 'retrieved_context', 'ai_response', 'moderation_flags', 'response_time_ms']
...
def upload_log_to_hf():
    if not HF_TOKEN or not HF_REPO_ID:
        print("Subida omitida: Faltan las variables de entorno de Hugging Face.")
        return
    try:
        hf_api.upload_file(path_or_fileobj=LOG_FILE, path_in_repo="log.csv", repo_id=HF_REPO_ID, repo_type="dataset")
        print(f"Log subido exitosamente a {HF_REPO_ID}")
    except Exception as e:
        print(f"ERROR: Falló la subida del log a Hugging Face: {e}")
...
def ask_candidato_ia(pregunta):
    ...
    3.  **GUARDARRAÍL DE REALIDAD:** Construye tu respuesta usando únicamente las ideas y propuestas presentes en la "Información de Apoyo". No introduzcas programas, planes o promesas nuevas que que no estén en el texto.
    ...
    """
    try:
        res_completion = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "user", "content": prompt_template}], temperature=0.4)
...
def whatsapp_reply():
    ...
    # --- FIN DE LA RED DE SEGURIDAD ---
        ...
        # Si CUALQUIER error no esperado ocurre, lo registraremos en los logs
        # y enviaremos una respuesta de error al usuario para que no se quede esperando.
