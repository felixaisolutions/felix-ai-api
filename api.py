# ==============================================================================
# FELIX AI SOLUTIONS - API DEL CANDIDATO DIGITAL (v31.0 "PRODUCCIÓN FINAL")
# Combina la arquitectura de alto rendimiento "Fire and Forget" con el
# "Prompt Maestro" refinado para una estabilidad y calidad de respuesta óptimas.
# ==============================================================================
...
"""
MODERATION_FALLBACK_MESSAGE = "Mi propósito es mantener una conversación respetuosa y centrada en las propuestas. No puedo responder a mensajes que contienen lenguaje ofensivo. Si tienes alguna pregunta sobre el plan de gobierno, estaré encantado de responder."
FALLBACK_RESPONSES = [
...
def append_log_to_r2(log_data):
    ...
    try:
        ...
        try:
            ...
            new_content = existing_object['Body'].read().decode('utf-8') + log_line
...
def ask_candidato_ia(pregunta):
    ...
    1.  **MANEJO DE AFIRMACIONES (Máxima Prioridad):** Si la pregunta del usuario es una afirmación sobre un hecho (ej: "¿es cierto que fue futbolista?", "usted es corrupto"), tu primera tarea es evaluar si la "Información de Apoyo" confirma o niega esta afirmación.
        *   Si la información de apoyo **contradice** la afirmación, o **no la menciona en absoluto**, tu primera oración DEBE ser una negación educada y directa. Ejemplos: "Esa información no es correcta.", "Mi trayectoria es diferente.".
        *   **SOLO DESPUÉS** de aclarar, puedes usar el contexto relevante para redirigir la conversación.

    2.  **MANEJO DE PREGUNTAS DE OPINIÓN ("qué piensas de..."):** Si la pregunta pide tu opinión sobre un tema amplio (ej: "qué piensas de la drogadicción"), y el contexto no menciona esa palabra exacta pero sí temas relacionados (ej: "microtráfico", "juventud"), **USA ESOS TEMAS RELACIONADOS** para construir una respuesta que refleje la postura del candidato. Sintetiza las propuestas existentes para responder a la intención.

    3.  **SÍNTESIS Y PROFUNDIDAD (Comportamiento por Defecto):** Para todas las demás preguntas, tu objetivo es ser un excelente comunicador. **LEE TODO EL CONTEXTO** proporcionado. **CONECTA LAS IDEAS** de múltiples fragmentos para dar una respuesta completa y elaborada. No te limites a parafrasear un solo trozo de información.
...
def whatsapp_reply():
    ...
    if int(media_items) > 0:
        ...
        else: # Guardia de Moderación como último filtro antes de la IA
            try:
                mod_response = openai.Moderation.create(input=incoming_msg)
                if mod_response['results'][0]['flagged']:
                    print(f"Mensaje flageado por moderación: {incoming_msg}")
                    resp_msg = MODERATION_FALLBACK_MESSAGE
                    log_data.update({'type': 'moderation_fallback', 'ai_response': resp_msg})
            except Exception as e:
                print(f"ERROR: Falló la llamada a la API de Moderación: {e}")

    if resp_msg: # Si alguna guardia se activó
    ...
    def log_task(response):
        log_data.update({
            'type': 'ia_response', 'retrieved_context': contexto_recuperado,
            'ai_response': respuesta_ia, 'response_time_ms': int((time.time() - start_time) * 1000)
        })
        Thread(target=append_log_to_r2, args=(log_data,)).start()
        return response
