# ==============================================================================
# FELIX AI SOLUTIONS - API DEL CANDIDATO DIGITAL (VERSIÓN 18.0 - PERSISTENCIA ACUMULATIVA)
# Lógica de registro rediseñada para ser acumulativa y sin estado.
# ==============================================================================
...
# --- FUNCIÓN DE REGISTRO REDISEÑADA ---
def update_cumulative_log_on_hf(new_log_data):
    """
    Descarga el log existente de HF, añade la nueva entrada y lo vuelve a subir.
    Este es el nuevo núcleo de nuestra persistencia de datos.
    """
    if not HF_TOKEN or not HF_REPO_ID:
        print("Actualización de log omitida: Faltan las variables de entorno de Hugging Face.")
        return

    repo_filename = "log.csv"
    headers = ['timestamp', 'user_id', 'user_query', 'retrieved_context', 'ai_response', 'moderation_flags', 'response_time_ms']

    try:
        # 1. Descargar el archivo existente
        ...
        # Si el archivo no existe (primera ejecución), creamos un DataFrame vacío
        print(f"Log no encontrado en HF, creando uno nuevo. Error: {e}")
    ...
    # 3. Preparar y subir el archivo actualizado
    try:
        # Convertimos el DataFrame a un string CSV en memoria
        csv_buffer = StringIO()
        df_updated.to_csv(csv_buffer, index=False)
        csv_buffer.seek(0)
        
        # Subimos el contenido del string como un archivo
        hf_api.upload_file(
        ...
        print(f"Log acumulativo subido exitosamente a {HF_REPO_ID}. Total de entradas: {len(df_updated)}")
    except Exception as e:
        print(f"ERROR: Falló la subida del log acumulativo a Hugging Face: {e}")

# ... (cargar_y_verificar_cerebro y ask_candidato_ia no cambian) ...
...
def whatsapp_reply():
    try:
        ...
        log_data = {
            ...
            'ai_response': respuesta_ia, 'moderation_flags': "NOT_IMPLEMENTED",
            'response_time_ms': response_time_ms
        }
        
        # --- LÓGICA DE REGISTRO MODIFICADA ---
        # Ahora llamamos a nuestra nueva función acumulativa
...
# Ya no necesitamos inicializar un archivo local
# initialize_log_file()