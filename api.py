# ==============================================================================
# FELIX AI SOLUTIONS - API DEL CANDIDATO DIGITAL (VERSIÓN 15.0)
# Backend Robusto con Moderación, Límite de Tasa y Control de Longitud
# ==============================================================================
...
# --- CONFIGURACIÓN GLOBAL Y CLIENTES ---
...
# --- MEJORA V15.0: VARIABLES PARA LIMITACIÓN DE TASA ---
# Diccionario para almacenar el conteo de mensajes de los usuarios
user_message_counts = {}
MESSAGE_LIMIT = 20 # Límite de mensajes por usuario
TIME_WINDOW_HOURS = 24 # En una ventana de 24 horas

# --- LÓGICA DE CARGA DEL CEREBRO ---
def cargar_y_verificar_cerebro():
    print("Verificando el estado del cerebro...")
    campaign_document = """
...
Tu apoyo, tu voz y tus ideas son cruciales. Con tu participación, construiremos el Quindío que merecemos.
    ...
    if collection.count() != len(text_chunks):
        print("Recargando la base de datos...")
        ...
        print("El cerebro está actualizado.")

# --- NÚCLEO CONVERSACIONAL Y DE MODERACIÓN ---

def moderar_mensaje(mensaje):
    """Usa la API de Moderación de OpenAI para detectar contenido inapropiado."""
    try:
        response = openai.Moderation.create(input=mensaje)
        flagged = response["results"][0]["flagged"]
        return flagged
    except Exception as e:
        print(f"Error en la moderación: {e}")
        return False
...
def ask_candidato_ia(pregunta):
    ...
    2.  **AUMENTA CON HECHOS:** Enriquece tu respuesta mencionando nombres de programas específicos, cifras o datos concretos que encuentres en la "Información de Apoyo".
    3.  **GUARDARRAÍL DE REALIDAD:** Construye tu respuesta usando únicamente las ideas y propuestas presentes en la "Información de Apoyo". No introduzcas programas, planes o promesas nuevas que no estén en el texto.
    4.  **Ignora Errores Menores:** Céntrate en la intención principal de la pregunta.
    5.  **Regla de Oro:** Si la información es irrelevante, responde: "No estoy seguro de haber entendido bien tu pregunta. ¿Podrías reformularla de una manera más clara?".
    ---
    ...
    """
    try:
        res_completion = openai.ChatCompletion.create(
            model="gpt-4",
...
# --- ENDPOINT DE TWILIO CON CAPAS DE SEGURIDAD ---

@app.route("/whatsapp", methods=['POST'])
def whatsapp_reply():
    """Recibe y gestiona todos los mensajes entrantes de WhatsApp."""
    resp = MessagingResponse()
    
    # --- Capa 1: Filtro de Multimedia ---
    num_media = int(request.values.get("NumMedia", 0))
    if num_media > 0:
        resp.message("¡Gracias por compartir! Por el momento, solo puedo procesar mensajes de texto. Por favor, escríbeme tu consulta.")
        return str(resp)
        
    incoming_msg = request.values.get('Body', '').strip()
    user_number = request.values.get('From', '')

    # --- Capa 2: Límite de Tasa por Usuario ---
    current_time = datetime.now()
    if user_number in user_message_counts:
        user_data = user_message_counts[user_number]
        if current_time - user_data['timestamp'] > timedelta(hours=TIME_WINDOW_HOURS):
            user_data['conteo'] = 1
            user_data['timestamp'] = current_time
        else:
            if user_data['conteo'] >= MESSAGE_LIMIT:
                resp.message("Has alcanzado el límite de 20 mensajes por hoy. ¡Podemos seguir conversando mañana!")
                return str(resp)
            else:
                user_data['conteo'] += 1
    else:
        user_message_counts[user_number] = {'conteo': 1, 'timestamp': current_time}

    # --- Capa 3: Guardia de Longitud ---
    MAX_MSG_LENGTH = 1000
    if len(incoming_msg) > MAX_MSG_LENGTH:
        resp.message("Gracias por tu mensaje. Para poder ayudarte mejor, por favor envía preguntas más cortas y directas.")
        return str(resp)

    # --- Capa 4: Muro de Moderación ---
    if moderar_mensaje(incoming_msg):
        resp.message("Mi propósito es mantener una conversación respetuosa. No responderé a mensajes con lenguaje inapropiado.")
        return str(resp)

    # Si el mensaje es válido, procede con la IA
    if not incoming_msg:
        resp.message("Hola! Soy Javier Montoya, tu candidato digital. ¿En qué puedo ayudarte?")
        return str(resp)
    
    print(f"Mensaje válido recibido de {user_number}: {incoming_msg}")
    
    respuesta_ia = ask_candidato_ia(incoming_msg)
    resp.message(respuesta_ia)
    print(f"Respuesta de IA enviada a {user_number}")
...
if __name__ == "__main__":
    ...
    print(f"API iniciada en el puerto {port}")
