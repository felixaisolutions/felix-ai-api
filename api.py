# ==============================================================================
# FELIX AI SOLUTIONS - API DEL CANDIDATO DIGITAL (v27.0 "RENDIMIENTO EXTREMO")
# Arquitectura final con registro asíncrono en Cloudflare R2 para máxima velocidad.
# ==============================================================================
...
if all([R2_ACCOUNT_ID, R2_BUCKET_NAME, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY]):
    ...
    print("ADVERTENCIA: Faltan variables de entorno de R2. El logging estará desactivado.")
...
# --- FUNCIÓN DE LOGGING ASÍNCRONO EN R2 ---
def append_log_to_r2(log_data):
    if not s3_client:
        print("Logging en R2 omitido: cliente no configurado.")
        return
    try:
        today = datetime.utcnow().strftime('%Y-%m-%d')
        log_filename = f"logs_{today}.txt"
        log_line = json.dumps(log_data) + '\n'
        
        try:
            existing_object = s3_client.get_object(Bucket=R2_BUCKET_NAME, Key=log_filename)
        ...
        s3_client.put_object(
            Bucket=R2_BUCKET_NAME, Key=log_filename, Body=new_content.encode('utf-8'), ContentType='text/plain'
        )
        print(f"Log añadido exitosamente a R2 en {log_filename}")
    except Exception as e:
        print(f"ERROR CRÍTICO al registrar en R2: {e}")
...
def ask_candidato_ia(pregunta):
    ...
    prompt_template = f"""
    Tu Persona: Eres Javier Montoya. Responde siempre en primera persona ("Mi propuesta es..."). Tu tono es el de un líder cercano y directo.
    Tu Misión: Responder la pregunta del ciudadano basándote ÚNICA y ESTRICTAMENTE en la "Información de Apoyo".
    Instrucciones Clave:
    1.  **REGLA DE ORO DE VERIFICACIÓN:** Si la pregunta del usuario es una pregunta de sí/no sobre un hecho personal (ej: '¿es cierto que usted fue X?'), y la 'Información de Apoyo' NO confirma ese hecho, DEBES empezar tu respuesta con "Esa información no es correcta." o "No, eso no es cierto.". **NO CONFIRMES NUNCA UN HECHO QUE NO ESTÉ EXPLÍCITAMENTE ESCRITO EN EL TEXTO DE APOYO.**
    2.  **Si la pregunta es sobre tu identidad (ej: 'eres una persona'), IGNORA la 'Información de Apoyo' y responde directamente que eres una persona real, Javier Montoya.**
    3.  Sintetiza y Conecta: Usa la 'Información de Apoyo' para dar una respuesta completa.
    4.  Brevedad Esencial: Sé conciso. Máximo 2 o 3 párrafos cortos.
    ---
...
def whatsapp_reply():
    ...
    # --- Bloque de Guardias de Seguridad de Respuesta Rápida ---
    ...
    if int(media_items) > 0:
        resp_msg = MEDIA_FALLBACK_MESSAGE
        log_data.update({'type': 'media_fallback', 'ai_response': resp_msg})
    # Guardia 2: Identidad
    elif any(keyword in incoming_msg.lower() for keyword in IDENTITY_KEYWORDS):
        resp_msg = IDENTITY_FALLBACK_MESSAGE
        log_data.update({'type': 'identity_fallback', 'ai_response': resp_msg})
    # Guardia 3: Saludo de Bienvenida
        ...
        if is_new_session and is_greeting:
            ...
            resp_msg = WELCOME_MESSAGE
            log_data.update({'type': 'welcome', 'ai_response': resp_msg})
        else:
            resp_msg = None

    # Si una guardia se activó, respondemos y salimos.
    if resp_msg:
        Thread(target=append_log_to_r2, args=(log_data,)).start()
        resp = MessagingResponse(); resp.message(resp_msg)
        return str(resp)
    ...
    # 1. ENVIAR RESPUESTA AL USUARIO INMEDIATAMENTE
    resp = MessagingResponse()
    resp.message(respuesta_ia)
    print(f"ENVIANDO a {user_number}: '{respuesta_ia[:50]}...'")
    
    # 2. PROGRAMAR LOGGING PARA DESPUÉS DE LA RESPUESTA
    @after_this_request
    def log_task(response):
        log_data.update({
            'type': 'ia_response',
            'retrieved_context': contexto_recuperado,
            'ai_response': respuesta_ia,
            'response_time_ms': int((time.time() - start_time) * 1000)
        })
        # El logging se ejecuta en segundo plano
        Thread(target=append_log_to_r2, args=(log_data,)).start()
        return response
